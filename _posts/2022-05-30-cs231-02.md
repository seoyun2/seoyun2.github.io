...
* 손실함수가 Hinge Loss 모양이라는 것을 생각해 보면 $\inty$, 0 <br>
...
* Loss가 0이 되려면 실제 스코어는 무한대에 가깝게 높아야하고 지수화와 정규화를 하기 때문에 확률 1(정답), 0(그외)를 얻으려면 정답 클래스의 스코어는 $+\inty$, 나머지는 $-\inty$이 되어야 함
* 최댓값은 없음 <br>
